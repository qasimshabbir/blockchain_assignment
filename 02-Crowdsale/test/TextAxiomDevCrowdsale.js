var AxiomDevCrowdsale = artifacts.require("AxiomDevCrowdsale");
var AxiomDevToken = artifacts.require("AxiomDevToken");

contract('AxiomDevCrowdsale', function(accounts) {
    it('should deploy the token and store the address', function(done){
        AxiomDevCrowdsale.deployed().then(async function(instance) {
    ...
    it('one ETH should buy 50 AxiomDev Tokens in PreICO', function(done){
        AxiomDevCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[7], value: web3.toWei(1, "ether")});
            const tokenAddress = await instance.token.call();
            const AxiomDevToken = AxiomDevToken.at(tokenAddress);
            const tokenAmount = await AxiomDevToken.balanceOf(accounts[7]);
            assert.equal(tokenAmount.toNumber(), 50000000000000000000, 'The sender didn\'t receive the tokens as per PreICO rate');
            done();
       });
    });
    ...
    it('should transfer the ETH to wallet immediately in Pre ICO', function(done){
        AxiomDevCrowdsale.deployed().then(async function(instance) {
            ...
            assert.equal(newBalanceOfBeneficiary, balanceOfBeneficiary + 20000000000000000000, 'ETH couldn\'t be transferred to the beneficiary');
            done();
       });
    });
    ...
    it('one ETH should buy 20 AxiomDev Tokens in ICO', function(done){
        AxiomDevCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[2], value: web3.toWei(1.5, "ether")});
            const tokenAddress = await instance.token.call();
            const AxiomDevToken = AxiomDevToken.at(tokenAddress);
            const tokenAmount = await AxiomDevToken.balanceOf(accounts[2]);
            assert.equal(tokenAmount.toNumber(), 30000000000000000000, 'The sender didn\'t receive the tokens as per ICO rate');
            done();
       });
    });
